...
| `GET /api/vault/search`                                            | Deterministic local vault search — `?query`, optional `type`, `status`, `tags`, `limit`; returns ranked results with snippets, matched fields, source path, and provenance                                    |
