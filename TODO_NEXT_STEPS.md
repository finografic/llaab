...
3. Decide the first extraction target:
...
4. Add a small “safe mutation” pattern for node edits:
   - read file
   - parse
   - validate
   - update
...
4. Decide how duplicate YouTube ingests should behave:
...
5. Decide the first stable source-linking rule:
   - always create or reuse one `source` node per channel
6. Decide the first transcript refinement rule:
   - metadata only
   - transcript + summary
   - transcript + extracted ideas
7. Add one simple CLI or script entry point for repeated use once the core helpers are ready.
...
4. then use those to harden YouTube ingestion as the first real feature
