...
- For in-app navigation use React Router `Link` / `useNavigate`; reserve native `<a href>` for external URLs. Feature **dialogs** live in `apps/client/src/dialogs/` (alongside `forms/` and `tables/` at `src/` root). **Form Labels:** all inputs use the shared `packages/ui` `Label` with `--text-muted` (dim grey token—no opacity stacking). **Inputs (global):** light `white/33`, dark `black/33` backgrounds in `packages/ui`. **Buttons (global):** `font-semibold` (600); slightly wider default horizontal padding except icon-only. **Drop zones (shared with Ingest):** accent border/tint only while dragging over—not on idle hover; keep the same 1px dashed border element (never dotted; no translate/box-shadow that shifts layout). **Tabs:** default to shadcn `variant="line"`, `text-base`, and slightly wider trigger padding (+0.25rem horizontal)—canonize as the shared `packages/ui` tabs default so every page matches; denser/smaller variants only when explicitly needed.
...
- **Grid layout:** Multi-column responsive page/section structure uses `Row`/`Col`/`Container` from `components/ui/grid` — see [`docs/components/grid.md`](docs/components/grid.md) and migration tracker [`docs/todo/TODO_GRID_LAYOUT_MIGRATION.md`](docs/todo/TODO_GRID_LAYOUT_MIGRATION.md). Converted examples: `crons.tsx` form row; `inbox.tsx` summary (`Col xs={6} md={3}`); registry detail main+sidebar (`Row nogutter`, **900px** stack breakpoint, **340px** sidebar); `TerminalPanel.tsx` actions/output (`Col lg="content"` 400px). Keep Tailwind micro-flex, `BalancedGrid`, `llm-card-grid`, and `PageLayout` CSS Grid as documented exceptions.
