...
- **Env / client / ports:** Monorepo `.env` at repo root; Vite `envDir` points there. `LLAAB_API_URL` proxies `/api` and `/terminal` only (not in the browser bundle); do not widen Vite `envPrefix` to expose server-named vars to the client bundle. `LLAAB_API_KEY`, `VAULT_PASSWORD`, OAuth/LLM keys are server-only—client uses same-origin paths. `VITE_*` exposes client bundle vars; unset `VAULT_PASSWORD` disables vault login. Local: client **5050** (`llaab.localhost:5050`), server **8888**, icons **5001**/**5199**. macOS launchd `com.llaab.client` runs `vite dev` (HMR), not `vite preview`.
