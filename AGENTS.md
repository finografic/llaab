...
- **Grid layout:** Multi-column responsive page/section structure uses `Row`/`Col`/`Container` from `components/ui/grid` — see [`docs/components/grid.md`](docs/components/grid.md) and [`docs/todo/DONE_GRID_LAYOUT_MIGRATION.md`](docs/todo/DONE_GRID_LAYOUT_MIGRATION.md). Converted examples: `crons.tsx` form row; `inbox.tsx` summary (`Col xs={6} md={3}`); registry detail main+sidebar (`Row nogutter`, **900px** stack breakpoint, **340px** sidebar); `TerminalPanel.tsx` actions/output (`Col lg="content"` 400px). Keep Tailwind micro-flex, `BalancedGrid`, `llm-card-grid`, and `PageLayout` CSS Grid as documented exceptions.
