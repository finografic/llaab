...
- **lean-ctx (not graphify):** graphify is removed—do not invoke graphify tools/rules. lean-ctx must not append savings/output footers into source or CSS (`--- lean-ctx: … ---` breaks PostCSS/Vite); keep `savings_footer=never`, `output_footer=never`, and `bypass_hints=off` in lean-ctx config, and strip any footers that land in files.
