...
- **Gotcha:** Monorepo `.env` lives at repo root; Astro SSR and `@llaab/ingestion` need explicit root env loading (`envDir` in `astro.config.ts`, `loadMonorepoEnv()`)—`apps/client` cwd does not pick it up automatically.
- **Gotcha:** React islands cannot share context (`QueryClientProvider`, etc.) across the `.astro` template boundary — Astro renders nested framework components to HTML independently of their wrapping component, so a provider declared around an island in `.astro` never reaches it. Wrap the provider _inside_ the island component, not around it in the template — see [`docs/astro/ASTRO_FETCH_AND_PAGES.md`](./docs/astro/ASTRO_FETCH_AND_PAGES.md#gotcha-providers-cannot-wrap-islands-across-the-astro-boundary).
