...
- Ingest `RunsTable`: grouped rows collapsed by default; child rows align to parent columns (not a single `colSpan`); dedicated **Nodes** column (`produced_node_ids.length` / group avg); group title cell is flex `space-between` with first-ingest date `dd-MM-YYYY` right (`fmtClickDate`); child-row `ExtractionModelCard` uses `showTotalTokens={false}`.
...
- **Canonical consolidation:** single-pass on `consolidate` LLM task (default `?mode=single-26b`); `consolidate-audit` removed. Post-pass rules in `apps/server/src/routes/vault/consolidation-quality.ts`; API returns `qualityValidation` (percentage score); transcript UI shows score on the Canonical Ideas bar.
