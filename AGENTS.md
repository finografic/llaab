...
- Vault entity **detail** routes wrap body content in `PageDetail`; vault **list** routes use `PageList`—except `/vault/transcripts` (+ `/:id`), which share `TranscriptsSplitView` (sidebar split + empty state when unselected). Set route handle `{ fullBleed: true }` on transcript routes for full-height split views.
...
- Tooling: ESLint removed (oxlint + oxfmt for TS/JS, Prettier for Markdown and legacy formats where needed); TypeScript pinned to 6.x—set explicit `compilerOptions.types` (base `["node"]`, `apps/server` `["node", "bun"]`).
...
- **Local ports:** client (Vite) on **3000** (`llaab.localhost:3000`); server (Bun API) on **8888**; icons on **5001** / **5199**. Set `PORT` + `LLAAB_API_URL` in `.env` or launchd plists. macOS launchd `com.llaab.client` runs `vite dev` (HMR)—keep that over `vite preview` for local iteration.
- **SPA routing:** `apps/client` is a Vite + React Router SPA. Routes live in `src/routes/` and are wired in `src/router.tsx`. Vault routes use `vaultSessionLoader` + nested `VaultLayout`; set `handle: { title, fullBleed? }` for `AppLayout` chrome. Data fetching uses TanStack Query hooks in `src/queries/` with a single root `QueryClientProvider` in `main.tsx`; do not pass `initialData: []` (legacy Astro SSR carryover—it marks queries fresh and can skip `/api/*` fetches).
