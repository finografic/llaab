...
- **`/llm` page:** one responsive model grid for local + cloud cards (`minmax(min(100%, 700px), 1fr)`; page ~1600px max). OpenCode Go uses **fuchsia** provider tokens in `ai-model-info.tsx`. Inline loader in `PageHero` meta while `/api/llm/status` loads.
