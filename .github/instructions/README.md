...
| `docs/`    | Documentation standards, README rules, agent-facing markdown, design specs |
| `git/`     | Commit conventions, branch policy, release process                         |
| `project/` | LLAAB-specific constraints — not reusable outside this repo                |
...
3. If the rule applies only to LLAAB (not a general convention), put it in `project/`.
...
- **`project/`** is strictly for LLAAB-specific constraints. Everything else is reusable convention.
- **Don't add an index here.** `/AGENTS.md` is the cross-agent entry point. `/AGENTS.md` lists paths; this README explains structure.
