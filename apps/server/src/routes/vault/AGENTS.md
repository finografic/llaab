...
| `vault-git.routes.ts`         | Git status       | `vaultGitStatus`, `vaultGitCommit`, `vaultGitReset`                                                                                                                                                       |
| `vault-nodes.routes.ts`       | Node CRUD/Search | `vaultTree`, `cleanRecent`, `file`, `listVaultNodes`, `searchVault`, `createVaultNode`, `createVaultResourceNode`, `batchUpdateVaultNodes`, `nodeDetail`, `updateVaultNode`, `deleteVaultNode`, `nodeRaw` |
| `vault-transcripts.routes.ts` | Transcripts      | `transcriptIdeas`, `extractTranscript`, `consolidateTranscriptIdeas`, `resolveCanonicalIdeaConflict`, `cleanCanonicalIdeaArtifacts`, `promoteCanonicalIdea`, `discardTranscript`                          |
