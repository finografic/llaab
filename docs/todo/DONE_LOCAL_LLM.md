# DONE — LLM Communication Layer (Local + Remote)

> **Completed:** 2026-04-16 — `@llaab/llm` package with Anthropic + Ollama providers, task router,
> 24h cache; `/api/llm/complete`, `/api/llm/stream`, `/api/llm/models` routes live in `apps/server`.
> Switched Ollama provider to `chat` API (2026-04-19) for proper system/user message separation.
