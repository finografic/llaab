...
| Route                     | Description                                                                         |
| ------------------------- | ----------------------------------------------------------------------------------- |
| `/ingest`                 | YouTube URL form + TagsInput → `POST /api/ingest/youtube`; two-phase status display |
| `/vault`                  | Gated file-tree browser — raw vault file viewer                                     |
| `/vault/transcripts/[id]` | Detail: source metadata, summary, extracted ideas (linked), full transcript         |
...
| Route                      | Description                                                                              |
| -------------------------- | ---------------------------------------------------------------------------------------- |
| `POST /api/ingest/youtube` | `ingestYouTube` skill — `{ url, title?, tags? }`; returns transcript + extraction status |
...
2. `extractKnowledgeFromTranscript(id, path, plainText)` — runs `llmExtractWithTrace`, updates transcript `summary`, creates `IdeaNode`s.

LLM tier for extraction: `local-mid` → default `llama3.2:3b`. Override with `LLAAB_LOCAL_MID_MODEL`.
...
| extract | local-mid   | llama3.2:3b       |
| code    | local-mid   | llama3.2:3b       |
...
- LLM dev tools: surface `GET /api/llm/models` in UI (P3).
